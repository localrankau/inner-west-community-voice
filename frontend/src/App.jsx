...
// Social icons not in newer lucide-react — use inline SVG
...
function loadStoredVotes() {
  try {
    return JSON.parse(localStorage.getItem("iwcv_votes") || "{}");
  } catch {
    return {};
  }
}
...
// HOME PAGE
// ────────────────────────────────────────────────────────────────────────────
function HomePage({ issues, loading, error, onRetry, onOpenIssue, onVote, userVotes, onPost, onShare }) {
    ...
    <main>
      ...
      {error && (
        <div style={{ textAlign: "center", padding: "32px 24px", color: COLORS.error, fontSize: 15 }}>
          ...
          <button
            onClick={onRetry}
            style={{ marginLeft: 8, color: COLORS.authority, fontWeight: 600, textDecoration: "underline" }}
          >
            Retry
          </button>
        </div>
...
function Hero({ query, setQuery, stats, onPost }) {
  return (
    ...
    >
      <div style={{ position: "absolute", inset: 0, backgroundImage: `url(${HERO_IMAGE})`, backgroundSize: "cover", backgroundPosition: "center 45%", transform: "scale(1.05)" }} />
      ...
      <div style={{ position: "relative", maxWidth: 1180, margin: "0 auto", padding: "80px 24px", width: "100%" }}>
        <div
          style={{
            ...
            borderRadius: 999, background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.2)",
        ...
        <h1
          className="serif"
          style={{ fontSize: "clamp(36px, 6vw, 64px)", fontWeight: 500, lineHeight: 1.05, margin: 0, letterSpacing: "-0.025em", maxWidth: 900 }}
        >
        ...
        <div
          style={{
            ...
            background: "white", borderRadius: 10, padding: "6px 6px 6px 16px",
            maxWidth: 560, boxShadow: "0 12px 30px rgba(0,0,0,0.25)",
          }}
        >
          ...
          <input
            ...
            style={{ border: "none", padding: "10px 4px", fontSize: 15, background: "transparent", color: COLORS.ink, boxShadow: "none" }}
          />
          <button
            onClick={onPost}
            style={{ background: COLORS.community, color: "white", fontWeight: 600, padding: "10px 16px", borderRadius: 6, fontSize: 14, whiteSpace: "nowrap" }}
          >
            Post issue
          </button>
        </div>
        ...
        return (
          <button
            ...
            style={{
              ...
              alignItems: "center", padding: "18px 20px", width: "100%", textAlign: "left",
          ...
          >
            <div className="serif" style={{ fontSize: 28, fontWeight: 500, color: idx === 0 ? COLORS.gold : COLORS.slate, fontStyle: "italic", letterSpacing: "-0.02em" }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 12, color: COLORS.slate }}>
                  <MapPin size={11} style={{ display: "inline", verticalAlign: "-1px", marginRight: 2 }} />{issue.suburb}
                </span>
                {issue.escalated && (
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: COLORS.ink, lineHeight: 1.35, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical" }}>
                {issue.title}
              </div>
              <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: 12, color: COLORS.slate, whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }}>
                  {issue.vote_count || 0} / {ESCALATION_THRESHOLD}
                </span>
              </div>
            </div>
  ...
  return (
    <div style={{ marginTop: 32, padding: "56px 24px", textAlign: "center", background: COLORS.paper, border: `1px dashed ${COLORS.hairline}`, borderRadius: 12 }}>
      <div className="serif" style={{ fontSize: 22, fontWeight: 500, marginBottom: 8 }}>
        {hasIssues ? "No issues match your filters" : "No issues yet"}
      </div>
      <p style={{ color: COLORS.slate, marginBottom: 20 }}>
        {hasIssues ? "Try a different suburb or category." : "Be the first to raise one for your suburb."}
      </p>
      <button onClick={onPost} style={{ background: COLORS.community, color: "white", fontWeight: 600, padding: "12px 20px", borderRadius: 6, fontSize: 14 }}>
    ...
    <section style={{ position: "relative", overflow: "hidden", borderRadius: 16, margin: "0 24px 80px" }}>
      ...
      <div style={{ position: "relative", padding: "56px 40px", color: "white", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 24 }}>
        <div style={{ maxWidth: 560 }}>
          ...
          <h3 className="serif" style={{ fontSize: "clamp(26px, 3.4vw, 36px)", fontWeight: 500, margin: 0, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
        ...
        <button
          onClick={onPost}
          style={{ background: COLORS.gold, color: COLORS.ink, fontWeight: 700, padding: "16px 28px", borderRadius: 8, fontSize: 15, display: "inline-flex", alignItems: "center", gap: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.25)", transition: "transform 150ms ease" }}
...
function IssueCard({ issue, onOpen, onVote, userVote, onShare, index = 0 }) {
  ...
  return (
    <article
      style={{
        background: COLORS.paper, borderRadius: 12, border: `1px solid ${COLORS.hairline}`,
        padding: 20, display: "flex", flexDirection: "column",
        boxShadow: "0 1px 2px rgba(0,0,0,0.03)", transition: "transform 200ms ease, box-shadow 200ms ease",
        animation: `cardEnter 400ms ease ${index * 40}ms both`, position: "relative",
      }}
    ...
    >
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: COLORS.slate, fontWeight: 500 }}>
          <MapPin size={12} />{issue.suburb}
        </span>
        {issue.escalated && (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 4, background: "#E6F4EA", color: COLORS.success, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>
            <CheckCircle2 size={11} /> Escalated
          </span>
        )}
        {!issue.escalated && pct >= 70 && (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 4, background: "#FEF3E2", color: "#9A4D07", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>
      ...
      <button onClick={onOpen} style={{ textAlign: "left", padding: 0, marginBottom: 8 }}>
        <h3 className="serif" style={{ fontSize: 20, fontWeight: 600, lineHeight: 1.25, margin: 0, color: COLORS.ink, letterSpacing: "-0.01em" }}>
          {issue.title}
        </h3>
      </button>

      <p style={{ fontSize: 14, lineHeight: 1.55, color: COLORS.slate, margin: "0 0 16px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
        {issue.description}
      </p>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: COLORS.ink, fontVariantNumeric: "tabular-nums" }}>
            {(issue.vote_count || 0).toLocaleString()} <span style={{ color: COLORS.slate, fontWeight: 500 }}>/ {ESCALATION_THRESHOLD}</span>
          </span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: "auto" }}>
        ...
        <div style={{ position: "relative", marginLeft: "auto" }}>
          <button
            ...
            style={{ padding: "9px 12px", border: `1px solid ${COLORS.hairline}`, borderRadius: 6, background: COLORS.paper, color: COLORS.ink, fontSize: 13, fontWeight: 500, display: "inline-flex", alignItems: "center", gap: 6, transition: "background 150ms ease" }}
        ...
        <button onClick={onOpen} style={{ padding: "9px 12px", background: "transparent", color: COLORS.authority, fontSize: 13, fontWeight: 600 }}>
          View →
        </button>
      </div>
    </article>
    ...
    <div ref={ref} style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, background: COLORS.paper, border: `1px solid ${COLORS.hairline}`, borderRadius: 8, boxShadow: "0 12px 28px rgba(0,0,0,0.14)", padding: 6, zIndex: 10, minWidth: 170, animation: "fadeIn 150ms ease" }}>
      {items.map((it) => (
        <a key={it.label} href={it.href} target="_blank" rel="noreferrer"
          style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 4, fontSize: 13, color: COLORS.ink, textDecoration: "none" }}
          onMouseEnter={(e) => (e.currentTarget.style.background = COLORS.mist)}
          onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
        >
          {it.icon}{it.label}
      ...
      <button
        onClick={() => { onShare(issue); onClose(); }}
        style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 4, fontSize: 13, color: COLORS.ink, width: "100%", textAlign: "left", borderTop: `1px solid ${COLORS.hairline}`, marginTop: 2 }}
        onMouseEnter={(e) => (e.currentTarget.style.background = COLORS.mist)}
        onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
      >
...
// ISSUE DETAIL PAGE
// ────────────────────────────────────────────────────────────────────────────
function IssueDetailPage({ issue, comments, onBack, onVote, userVote, onComment, onSignup, hasSignedUp, onShare, onFetchComments }) {
  const pct = Math.min(100, ((issue.vote_count || 0) / ESCALATION_THRESHOLD) * 100);
    ...
    <main style={{ paddingBottom: 100 }}>
      <div style={{ background: COLORS.paper, borderBottom: `1px solid ${COLORS.hairline}` }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 24px 0" }}>
          <button onClick={onBack} style={{ display: "inline-flex", alignItems: "center", gap: 6, color: COLORS.slate, fontSize: 13, fontWeight: 500, padding: "6px 0", marginBottom: 20 }}>
            <ArrowLeft size={14} /> All issues
          </button>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
            <CategoryBadge category={issue.category} />
            <span style={{ fontSize: 13, color: COLORS.slate }}>
              <MapPin size={13} style={{ display: "inline", verticalAlign: "-2px", marginRight: 3 }} />{issue.suburb}
            </span>
            <span style={{ fontSize: 13, color: COLORS.slate }}>· Posted {timeAgo(issue.created_at)}</span>
          </div>
          <h1 className="serif" style={{ fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 500, lineHeight: 1.12, margin: "0 0 24px", letterSpacing: "-0.02em", color: COLORS.ink }}>
            {issue.title}
          </h1>
        </div>
      </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
            <div>
              <div className="serif" style={{ fontSize: 32, fontWeight: 600, lineHeight: 1, letterSpacing: "-0.02em", color: COLORS.ink }}>
                {(issue.vote_count || 0).toLocaleString()}
    ...
    <section style={{ marginBottom: 36, padding: 24, background: COLORS.paper, border: `1px solid ${COLORS.hairline}`, borderRadius: 12, borderTop: `3px solid ${COLORS.community}` }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <button
          type="submit"
          disabled={submitting}
          style={{ marginTop: 4, padding: "12px 18px", background: submitting ? COLORS.slate : COLORS.community, color: "white", fontWeight: 600, borderRadius: 6, fontSize: 14, display: "inline-flex", alignItems: "center", gap: 8, justifySelf: "start", cursor: submitting ? "not-allowed" : "pointer" }}
    ...
    <form onSubmit={handleSubmit} style={{ background: COLORS.paper, border: `1px solid ${COLORS.hairline}`, borderRadius: 10, padding: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 10, flexWrap: "wrap" }}>
        ...
        <button
          type="submit"
          disabled={!text.trim()}
          style={{ marginLeft: "auto", padding: "9px 14px", background: text.trim() ? COLORS.authority : COLORS.slate, color: "white", fontWeight: 600, borderRadius: 6, fontSize: 13, cursor: text.trim() ? "pointer" : "not-allowed", opacity: text.trim() ? 1 : 0.55 }}
        >
          Post comment
        </button>
      </div>
    ...
    <div style={{ background: COLORS.paper, border: `1px solid ${COLORS.hairline}`, borderRadius: 10, padding: "14px 16px" }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 6 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: COLORS.ink }}>
          {comment.anonymous || (!comment.commenter_name && !comment.name) ? "Anonymous" : (comment.commenter_name || comment.name)}
        </span>
...
function PostIssueModal({ onClose, onSubmit }) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(6,37,71,0.55)", backdropFilter: "blur(6px)", WebkitBackdropFilter: "blur(6px)", zIndex: 100, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "40px 16px", overflowY: "auto", animation: "fadeIn 200ms ease" }}
      onClick={onClose}
    >
      <form
        onClick={(e) => e.stopPropagation()}
        onSubmit={handleSubmit}
        style={{ background: COLORS.paper, borderRadius: 14, padding: 28, maxWidth: 580, width: "100%", boxShadow: "0 24px 60px rgba(0,0,0,0.35)", animation: "cardEnter 300ms ease" }}
      >
